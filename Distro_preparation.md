...
Предыдущая статья: [Setup_plan](Setup_plan.md)

Начнем с дистрибутива. На самом деле, описанные в этом гайде инструкции можно выполнять практически на любом дистрибутиве linux. Я рекомендую Arch или любой другой Arch-based дистр, а лучше всего Artix, по одной простой причине - в этих дистрибутивах апстримное ПО, т.е. в отличии от какого-нибудь Ubuntu/Debian вам не нужно ждать, пока ПО выйдет, его проверят, пропатчат, полностью поменяют формат и местоположение конфигов. Данный гайд построен на актуальной и оригинальной версии ПО (на момент написания), поэтому на downstream-дистрах у вас могут быть проблемы.
...
В этом гайде для управления процессами пользователя будет использоваться s6, Однако, в отличии от systemd и openrc, s6 можно запускать от имени обычного пользователя, сколько угодно раз, и даже имея другой инит в системе. То есть да, она может быть использована как pid 1, но это не обязательно (а для данного гайда вообще не нужно). s6 есть в официальных репозиториях artix, а так же в aur ([s6](https://aur.archlinux.org/packages/s6), [s6-rc](https://aur.archlinux.org/packages/s6-rc)) если у вас arch (установка s6 на арч НЕ сломает systemd и не будет с ней конфликтовать, не переживайте).

# Установка

В общем-то, вам будет достаточно "голого" Arch или Artix. Во втором случае не имеет значения какой init выбираете, об этом было выше.

Если у вас уже есть готовая система которую не жалко ломать, то можно делать на ней. Наличие какого-либо софта, DE, других графических программ не имеет значения и никак не мешает, главное - чтобы система запускалась и был доступ в Интернет. Если же вы хотите начать с нуля:
...
NetworkManager слишком overhead для ADHDE, но опять же - выбирать вам. Но в целом вам iwd должно хватить для "включил ноут, подключился к wifi, если сеть новая - ввёл пароль".
