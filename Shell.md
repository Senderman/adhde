...
Использование аналогично как у sudo:
```bash
sudo cmd # выполнить команду от рута
sudo -u anotheruser cmd # выполнить команду от другого юзера
sudo -s # провалиться в shell рута
sudo -u anotheruser -s # провалиться в вshell другого пользователя
```
...
Помните предыдущую статью про dotfiles? Давайте начнем сразу наполнять наш dotfiles репозиторий!
Перейдите в `~/dotfiles/` и создайте директорию `zsh`. В ней 2 пустых файла `.zprofile` и `.zshrc`. Затем, всё еще находясь в `~/dotfiles/`, выполните команду

```bash
stow zsh
```

Если вы всё сделали правильно, то `ls -al ~` должна показать следующее:
```bash
.zprofile -> dotfiles/zsh/.zprofile
.zshrc -> dotfiles/zsh/.zshrc
```

`~/.zprofile` заполним вот так (можете поменять на свой вкус):

```bash
 # если вы хотите хранить какие-то скрипты и бинари в домашней директории. Скрипты мы точно будем хранить
export PATH=$PATH:$HOME/scripts:$HOME/.local/bin
# Вообще, эта переменная должна быть уже выставлена, но это происходит не всегда, поэтому лучше перестраховаться
...
# Пригодится в будущем для ssh агента, т.к. запускать мы его будем сами
export SSH_AUTH_SOCK="/run/user/$(id -u)/ssh-agent"
# Если вы используете самоподписанные сертификаты, имеет смысл указать, чтобы приложения в питоне работали корректно
export REQUESTS_CA_BUNDLE=/etc/ssl/certs/ca-certificates.crt
# Можно не указывать, если не будете использовать свою тему для курсора мыши
...
Создайте файл `~/dotfiles/zsh/.zsh_aliases.txt`, затем в `~/dotfiles/` выполните снова `stow zsh`, чтобы создать симлинк (не забываем, что нам нужно всегда хранить все свои конфиг файлы внутри `~/dotfiles/`, чтобы можно было отслеживать версии, откатывать и хранить в удаленной гит-системе (github/gitab/codeberg)).


в `~/.zshrc` добавьте следующие строки:
...
файл `~/.zsh_aliases-nogit.zsh` в `~/dotfiles/` класть не нужно, это как раз сделано на случай, если у вас есть алиасы, которыми вы не хотите делиться, например, устройство не личное, а корпоративное.

теперь вы можете объявить алиасы в `~/.zsh_aliases.zsh`, они появятся при перезапуске терминала. Лично у меня вот такой набор:
...
Но zsh можно кастомизировать не только алиасами. Возможно, вы уже слышали про [Oh My Zsh](https://ohmyz.sh/). Однако, у этого проекта выполняется много кода при инициализации, и это будет замедлять каждую загрузку вашего шелла (т.е. каждое открытие терминала). Кроме того, существует потребность устанавливать плагины не из репозитория omz, а других. Ранее существовал менеджер плагинов Anitgen, написанный на zsh, но он так же не отличался по производительности. Затем появился Anitbody, написанный на go, чтобы решить проблему медленной инициализации. Но проект был заброшен, т.к. появились другие менеджеры плагинов, которые были еще быстрее. Проблема в том, что у antibody были фичи, которые понравились zsh-сообществу, и которых не было в других менеджерах. Поэтому и появился [antidote](https://antidote.sh/), который написан на zsh, решает многие проблемы медленной инициализации, а так же генерирует файл для статической загрузки плагинов, что сильно упрощает логику и количество кода, выполняемого при старте шелла.
...
Итак, создайте файл `~/dotfiles/zsh/.zsh_plugins.txt`, выполните `stow zsh`, и откройте `~/.zsh_plugins.txt` в редакторе. Мой рекомендуемый список плагинов:
...
Далее, добавьте в `~/.zshrc` следующие строки:
...
Добавьте в `~/.zshrc` и перезапустите терминал:
...
Добавьте **в конец (это очень важно!!!)** `.zshrc`:
