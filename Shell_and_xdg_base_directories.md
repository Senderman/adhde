...
Теперь вам нужно привыкнуть использовать команду `z` вместо `cd`. Но я сделал `alias z=cd` и за несколько месяцев использования не нашел нежелательных последствий данного действия.
