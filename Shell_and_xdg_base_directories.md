...
Теперь вам нужно привыкнуть использовать команду `z` вместо `cd`. Но я сделал `alias cd=z` и за несколько месяцев использования не нашел нежелательных последствий данного действия.
